...
class ServerConfig:
    ...
    def from_env(cls) -> "ServerConfig":
        ...
        return cls(
            ...
            auth=AuthConfig(
                ...
                profile_dir=os.getenv("NOTEBOOKLM_PROFILE_DIR", "./chrome_profile_notebooklm"),
                use_persistent_session=os.getenv("NOTEBOOKLM_PERSISTENT_SESSION", "true").lower() == "true",
            )
        )
    ...
    def validate(self) -> None:
        ...
        if self.auth.profile_dir and not Path(self.auth.profile_dir).parent.exists():
            raise ConfigurationError(f"Profile directory parent does not exist: {self.auth.profile_dir}")
